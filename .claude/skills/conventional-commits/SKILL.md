...
- `api`, `auth`, `db`, `ui`, `config`, `cli`, `core`. Optionnal, in parens — e.g. "feat(auth):"

### Subject

- Use **command tense** (imperative mood): "Add feature" not "Added feature" or "Adds feature"
- Keep it concise — **50 characters or fewer** (hard limit)
...
Include `Ref: <ticket>` whenever the commit is **substantive** — i.e. it introduces
or changes behavior, fixes a bug, or adds documentation that maps to a ticket.
Omit it for fixups, style tweaks, and WIP commits.

To find the ticket, extract it from the branch name (e.g. `feat/G3-145-foo` → `G3-145`).
If no ticket is identifiable, omit the footer rather than leaving a placeholder.
...
- [ ] Subject is concise and clear — **title is 50 characters or fewer**
...
- [ ] Commit represents a single, self-contained change
- [ ] Subject does not require "and" or "also" to describe the change
- [ ] Unrelated changes are not bundled (e.g. a fix and a style cleanup)
- [ ] Codebase is in a working state after this commit
...
0. **Verify atomicity** — confirm the commit represents one logical change.
   If it requires "and" or "also" to describe, split it before proceeding.
1. **Draft the commit message** following this guide
2. **Count the title characters** — must be 50 or fewer
3. **Add Ref footer if applicable** — extract ticket from the branch name (e.g. `G3-145`); omit if not identifiable
4. **Show the draft** to the user before committing
5. **Ask for feedback** if the commit is complex or touches multiple concerns
6. **Ensure the body adds value** - when in doubt, omit the body
7. **Include only relevant context** in the body
8. **NEVER add AI attribution** - no tool attribution of any kind
